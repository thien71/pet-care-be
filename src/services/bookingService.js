...
// ==================== SHOP BOOKINGS ====================
async function getShopBookings(userId, trangThai) {
...
async function assignTechnician(bookingId, maNhanVien) {
  const booking = await LichHen.findByPk(bookingId);
  if (!booking) {
    throw new Error("Booking not found");
  }
...
async function updateBookingStatus(bookingId, trangThai) {
  const booking = await LichHen.findByPk(bookingId);
  if (!booking) {
    throw new Error("Booking not found");
  }
...
// ==================== TECHNICIAN ====================
async function getMyAssignments(technicianId) {
  return await LichHen.findAll({
    where: {
